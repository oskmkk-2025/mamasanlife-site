...
async function fixNMNP() {
    console.log('Fetching mobile-number-portability post...');
    const post = await client.fetch(`*[_type == "post" && slug.current == "mobile-number-portability"][0]`);
    ...
    // Restore Nippon Tsushin SIM EasyLink (Moshimo EasyLink)
    // The user mentioned "NT-ST2-P" image is missing and wants smart display.
    const easyLinkKey = 'f41e527d7435_moshimo_0';
    const hasEasyLink = body.some(b => b._key === easyLinkKey || (b._type === 'htmlEmbed' && b.html.includes('NT-ST2-P')));

    if (!hasEasyLink) {
        console.log('Restoring Nippon Tsushin SIM EasyLink...');
        // Insert it after the block that mentions NT-ST2-P
        const idx = body.findIndex(b => b._type === 'block' && b.children.some(c => c.text.includes('NT-ST2-P')));
        if (idx !== -1) {
            body.splice(idx + 1, 0, {
                _type: 'htmlEmbed',
                _key: easyLinkKey,
                html: `<!-- START MoshimoAffiliateEasyLink -->
