...
export async function POST(req: Request){
  try{
    ...
    const q = `*[_type=='post' && defined(slug.current)]|order(title asc, _updatedAt desc)[0...$lim]{
    ...
    for (const [title, items] of byTitle){
      if (items.length > 1) groups.push({ title, items })
    }
    ...
    for (const g of groups){
      // choose primary: max blocks -> latest updatedAt
      ...
      const act:any = { title: g.title, keep: { slug: primary.slug, category: primary.category, blocks: primary.blocks }, remove: secondary.map(s=>({ id:s._id, slug:s.slug, category:s.category, blocks:s.blocks })) }
      results.push(act)
