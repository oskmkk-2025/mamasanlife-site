    ...
    <article className="border rounded-lg overflow-hidden bg-white/80 hover:shadow-sm transition-shadow">
      <Link href={href} className="block">
        <div className="relative w-full aspect-[16/9] bg-white/60">
          {imageUrl ? (
            <ImgWithPlaceholder src={sanityOptimized(imageUrl, { q: 70, fit:'crop' })} alt={title} fill sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw" className="object-cover" />
          ) : (
        ...
        <div className="p-4">
          ...
          <h3 className="text-lg font-semibold line-clamp-2 text-gray-900 card-title">{title}</h3>
          {excerpt && <p className="mt-2 text-sm text-gray-600 line-clamp-2 lg:line-clamp-3">{excerpt}</p>}
