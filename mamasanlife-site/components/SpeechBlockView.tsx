...
export function SpeechBlockView({ value }: { value: any }){
  ...
  useEffect(()=>{
    ...
    // 自動調整: 吹き出しの高さに応じて 56〜96px の範囲で調整（SPは最大72px）
    const isSP = typeof window !== 'undefined' && window.innerWidth <= 420
    const cap = isSP ? 72 : 96
    const size = Math.max(56, Math.min(cap, Math.round(h)))
    setIconSize(size)
  },[value?.paras])

  return (
    <div className={`speech-block my-5 flex ${alignRight ? 'justify-end' : 'justify-start'}`}>
      <div className={`max-w-[720px] w-full flex items-start gap-4 ${alignRight ? 'flex-row-reverse' : ''}`}>
        <div className={`flex flex-col items-center justify-start shrink-0 ${alignRight ? 'ml-2' : 'mr-2'}`} style={{ minHeight: iconSize }}>
          ...
          {value?.name && <div className="speech-name text-[11px] text-gray-600 mt-0 text-center leading-none max-w-[120px]">{value.name}</div>}
        </div>
        <div ref={bubbleRef} className="min-w-0">
          <div className={`speech-bubble relative px-4 py-3 rounded-2xl text-[15px] leading-relaxed`} style={{ background:'#fff', border:'1px solid var(--c-primary)' }}>
