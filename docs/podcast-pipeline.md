...
- **定期公開カレンダー（2026-07-22決定）**: ポッドキャスト=毎週金曜朝7時目安に1本（録音ストックの古い順）／ブログ連動記事=エピソードと同日／Instagram・X=水・土21時。進捗の正本は `~/claude/podcast/STOCK.md`（ep一覧の状態表。作業したら必ず更新）。
- **ストック運用**: 台本は常に3〜4本ストック（聞き取りは録音不要・いつでも可）。録音日に2〜3本まとめ録り、ボイスメモのファイル名にep番号（例: ep03）を入れてAirDrop。複数届いても公開は週1本ずつ。
